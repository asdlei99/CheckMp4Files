...
const UINT64 CH264AtomParser::GetDuration(const DWORD dwTrackId) const{
	...
	for(auto& TrackInfo : m_vTrackInfo){
		...
		if(TrackInfo.dwTrackId == dwTrackId){
			...
			if(TrackInfo.pCompositionShift == NULL){
				...
				DWORD dwTimeScale = TrackInfo.dwTimeScale ? TrackInfo.dwTimeScale : m_dwTimeScale;
				ui64Time = (UINT64)(((TrackInfo.pCompositionShift->lDisplayEndTime / (double)dwTimeScale) * 10000000));
			}
...
const DWORD CH264AtomParser::GetTrackSampleSize(const DWORD dwTrackId) const{
	...
	for(auto& TrackInfo : m_vTrackInfo){
		...
		if(TrackInfo.dwTrackId == dwTrackId){
			...
			dwSize = (DWORD)TrackInfo.vSamples.size();
			break;
		}
	}
